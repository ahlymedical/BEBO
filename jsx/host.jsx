...
function removeSilenceGaps(srtContent) {
  try {
    ...
    if (!seq) return JSON.stringify({ success: false, message: "لا يوجد سيكوانس نشط." });

    var entries = parseSRT(srtContent);
    if (entries.length === 0) return JSON.stringify({ success: false, message: "لم يُعثر على توقيتات." });
    ...
    if (gaps.length === 0) return JSON.stringify({ success: true, message: "لا توجد فراغات.", count: 0 });
    ...
    return JSON.stringify({
      success: true,
      message: "تم مسح " + removed + " فراغ من " + gaps.length,
      count: removed
    });
  } catch(e) {
...
function closeAllGaps() {
  try {
    ...
    return JSON.stringify({ success: true, message: "تم تجميع الكليبات! 🎉" });
  } catch(e) {
    return JSON.stringify({ success: false, message: "خطأ: " + e.toString() });
  }
}
